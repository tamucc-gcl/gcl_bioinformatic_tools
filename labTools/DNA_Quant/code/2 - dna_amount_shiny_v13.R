...
make_init_fn <- function(emp_priors, n_groups_1, n_groups_2) {
  function() {
    list(
...
inits_from_fit <- function(cmdstan_fit, n_chains, n_groups_1, n_groups_2) {
  s <- cmdstan_fit$summary(
    variables = c("Intercept", "b", "Intercept_shape", "b_shape", "sd_1", "sd_2"),
    "median"
  )
  ...
  model_fit <- eventReactive(input$fit_model, {
    tryCatch({
      ...
      sd_base <- make_standata(
        bf(as.formula(model_formula), shape ~ is_control + (1 | sample_id)),
        family = Gamma(link = 'log'),
        data = model_data,
