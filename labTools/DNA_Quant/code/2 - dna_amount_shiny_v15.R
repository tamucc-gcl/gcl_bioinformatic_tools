...
# Empirical priors derived from the actual y data
compute_empirical_priors <- function(y, is_control, sample_id) {
  is_control <- as.logical(as.character(is_control))
  pos <- y > 0 & !is.na(y)
  y_pos <- y[pos]
  log_y <- log(y_pos)
  
  is_ctrl_pos <- is_control[pos]
  log_y_sample <- log_y[!is_ctrl_pos]
  if (length(log_y_sample) < 3) log_y_sample <- log_y
...
# Reasonable starting values for each chain
make_init_fn <- function(emp_priors, n_groups_1, n_groups_2 = NA, shape_re = TRUE) {
  function() {
    init <- list(
      Intercept       = emp_priors$intercept_prior[1] + rnorm(1, 0, 0.2),
      b               = array(rnorm(1, 0, 0.3), dim = 1),
      Intercept_shape = emp_priors$inteceptShape_prior[1] + rnorm(1, 0, 0.2),
      b_shape         = array(rnorm(1, 0, 0.3), dim = 1),
      sd_1            = array(abs(rnorm(1, 0.3, 0.1)), dim = 1),
      z_1_raw         = array(rnorm(n_groups_1 - 1, 0, 0.1),
                              dim = c(1, n_groups_1 - 1))
    )
    if (shape_re) {
      init$sd_2    <- array(abs(rnorm(1, 0.3, 0.1)), dim = 1)
      init$z_2_raw <- array(rnorm(n_groups_2 - 1, 0, 0.1),
                            dim = c(1, n_groups_2 - 1))
    }
    init
  }
}
...
check_convergence <- function(fit) {
  s <- fit$summary(variables = c("Intercept", "b", "Intercept_shape",
                                 "b_shape", "sd_1", "sd_2"))
...
# Posterior medians → init values for a retry
inits_from_fit <- function(cmdstan_fit, n_chains, n_groups_1, n_groups_2 = NA, shape_re = TRUE) {
  vars <- c("Intercept", "b", "Intercept_shape", "b_shape", "sd_1")
  if (shape_re) vars <- c(vars, "sd_2")
  s <- cmdstan_fit$summary(variables = vars, "median")
  med <- setNames(s$median, s$variable)
  
  z1_med <- cmdstan_fit$summary(variables = "z_1_raw", "median")$median
  z2_med <- if (shape_re) cmdstan_fit$summary(variables = "z_2_raw", "median")$median else NULL
  
  lapply(seq_len(n_chains), function(i) {
    init <- list(
      Intercept       = unname(med["Intercept"]) + rnorm(1, 0, 0.05),
      b               = array(unname(med["b[1]"]) + rnorm(1, 0, 0.05), dim = 1),
      Intercept_shape = unname(med["Intercept_shape"]) + rnorm(1, 0, 0.05),
      b_shape         = array(unname(med["b_shape[1]"]) + rnorm(1, 0, 0.05), dim = 1),
      sd_1            = array(max(0.01, unname(med["sd_1[1]"])) + abs(rnorm(1, 0, 0.02)), dim = 1),
      z_1_raw         = array(z1_med + rnorm(length(z1_med), 0, 0.05),
                              dim = c(1, n_groups_1 - 1))
    )
    if (shape_re) {
      init$sd_2    <- array(max(0.01, unname(med["sd_2[1]"])) + abs(rnorm(1, 0, 0.02)), dim = 1)
      init$z_2_raw <- array(z2_med + rnorm(length(z2_med), 0, 0.05),
                            dim = c(1, n_groups_2 - 1))
    }
    init
  })
                     ...
                     bsCollapse(
                       bsCollapsePanel("Model Settings", 
                                       selectInput("fit_mode",
                                                   "Fitting mode:",
                                                   choices = c(
                                                     "Fast (no shape RE, Laplace)"  = "fast_laplace",
                                                     "Standard (no shape RE, NUTS)" = "fast_nuts",
                                                     "Full (shape RE, NUTS - increase sampling/iterations to 5,000+)"        = "full_nuts"
                                                   ),
                                                   selected = "fast_laplace"),
...
server <- function(input, output, session) {
  ...
  emp_priors_rv <- reactiveVal(NULL)
  ...
  filtered_data <- reactive({
    ...
    # DEBUG:
    # write_rds(reactiveValuesToList(input), 'input.rds')
    # write_rds(data, 'joined_data.rds')
      ...
      for (col in num_cols) {
        if (sum(!is.na(data[[col]])) > 0) {
          ...
          if (!is.null(range_input) && length(range_input) == 2) {
            ...
            lower_exceeds <- range_input[1] > original_range[1] + tol
            upper_exceeds <- range_input[2] < original_range[2] - tol
            
            if (lower_exceeds || upper_exceeds) {
              filtered <- filtered %>% 
  ...
  model_fit <- eventReactive(input$fit_model, {
    tryCatch({
      ...
      #DEBUG
      # write_rds(filtered_data(), 'filtered_data.rds')
      ...
      shape_re_val <- input$fit_mode == "full_nuts"
      use_laplace  <- input$fit_mode == "fast_laplace"
      ...
      if (shape_re_val) {
        ...
        # Simpler model: shape varies only by control vs sample
        dna_form <- bf(as.formula(model_formula),
                       shape ~ is_control)
      }
      ...
      # Compute empirical priors and inits from data  #NEW
      emp_priors <- compute_empirical_priors(model_data[[input$y_var]],
                                             model_data$is_control,
                                             model_data$sample_id)
      emp_priors_rv(emp_priors)
      
      # Build standata once and get group sizes from it (matches what Stan sees)
      sd_base <- make_standata(
        dna_form,                          # already built above with the right shape formula
        family = Gamma(link = 'log'),
        data = model_data,
      ...
      if(Sys.info()["nodename"] %in% c('gawain', 'lancelot') & Sys.info()["user"] != "jselwyn"){
        ...
        write_rds(model_data, './testing/model_data.rds')
        write_rds(data_stan, './testing/data_stan.rds')
        write_rds(init_fn, './testing/init_fn.rds')
      }
      ...
      sampling_function <- function(data_stan, output_dir, num_chains, iter_sampling,
        ...
        check_convergence_local <- function(fit) {
          all_vars <- fit$metadata()$stan_variables
          target_vars <- intersect(c("Intercept", "b", "Intercept_shape",
                                     "b_shape", "sd_1", "sd_2"), all_vars)
          s <- fit$summary(variables = target_vars)
          
          diag_sum <- fit$diagnostic_summary()
          treedepth_hits <- sum(diag_sum$num_max_treedepth)
          total_iter <- length(fit$draws()[, 1, 1])
        ...
        fit <- dna_model$sample(
          ...
          threads_per_chain = parallelly::availableCores() %/% 
            min(c(parallelly::availableCores(), num_chains)),
        ...
        list(files = fit$output_files(),
             diag = check_convergence_local(fit))
      }
      ...
      run_attempt <- function(num_chains, iter_warmup, iter_sampling, thin,
        ...
        withProgress(message = paste("Fitting model:", label), value = 0, {
          ...
          while (bg$is_alive()) {
            Sys.sleep(1)
            lines <- bg$read_output_lines()
            pcts <- suppressWarnings(as.numeric(gsub("%", "",
                                                     regmatches(lines, regexpr("[0-9]+%", lines)))))
      ...
      if ((Sys.info()["nodename"] %in% c('gawain', 'lancelot') & Sys.info()["user"] != "jselwyn") && !result$diag$ok) {
        ...
        prev_fit <- cmdstanr::as_cmdstan_fit(result$files)
        ...
        if (!result$diag$ok) {
          ...
          showNotification("Refit converged successfully.", type = "message", duration = 5)
        }
      }
      ...
      showNotification(paste("Model Fitting Error:", detail),
  ...
  plot_obj <- reactive({
    tryCatch({
      ...
      if (input$show_raw_points) {
        raw_points <- filtered_data() %>%
          mutate(dna_plate_well_id = str_extract(sample_id, '_[a-zA-Z][0-9]+$') %>%
                   str_remove('_'),
                 facet_plate = str_remove(sample_id, str_c('_', dna_plate_well_id))) %>%
          select(facet_plate, dna_plate_well_id, sample_type,
                 raw_value = !!sym(input$y_var)) %>%
          filter(raw_value > 0)  # log scale will drop zeros anyway
  ...
  observeEvent(input$generate_zip, {
    ...
    #### Output JSON ####
    model_details <- list(
      bayesian_fitting_settings = list('Fit Mode' = input$fit_mode,
                                       'Per-sample Shape RE' = input$fit_mode == "full_nuts",
                                       'Algorithm' = if (input$fit_mode == "fast_laplace") "Laplace" else "NUTS",
                                       'Number of Chains' = input$num_chains, 
      ...
      mean_priors  = emp_priors_rv()[c("intercept_prior", "beta_prior", "var_prior")],
      shape_priors = emp_priors_rv()[c("inteceptShape_prior", "betaShape_prior", "varShape_prior")],
    ...
    stan_filename <- if (input$fit_mode == "full_nuts") {
      "dna_concentration_threaded_sumtozero.stan"
    } else {
      "dna_concentration_threaded_sumtozero_noshapere.stan"
    }
    ...
    file.copy(stan_path, model_code, overwrite = TRUE)
