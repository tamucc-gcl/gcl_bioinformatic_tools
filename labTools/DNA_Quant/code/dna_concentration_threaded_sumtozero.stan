...
parameters {
  ...
  array[M_2] vector[N_2 - 1] z_2_raw;  // standardized group-level effects - sum to zero
}
transformed parameters {
  //chunk to reconstruct full vectors post sum-to-zero
  array[M_1] vector[N_1] z_1;
  array[M_2] vector[N_2] z_2;
  z_1[1, 1:(N_1 - 1)] = z_1_raw[1];
  z_1[1, N_1] = -sum(z_1_raw[1]);
  z_2[1, 1:(N_2 - 1)] = z_2_raw[1];
  z_2[1, N_2] = -sum(z_2_raw[1]);
...
model {
  ...
  target += std_normal_lpdf(z_2_raw[1]); // sum to zero change
}
