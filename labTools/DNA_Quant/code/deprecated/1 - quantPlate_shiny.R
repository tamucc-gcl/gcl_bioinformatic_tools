...
read_map_file <- function(path){
  ...
  if(any(str_detect(the_cols, '(sample)_id')) & 
    ...
    if (all(standards$plate_column < standards$plate_row)) {
      ...
      stop("No universal column where all values are smaller.")
    }
    ...
    stop(str_c("Problem with format of 'map_file': ", path))
  }
...
#### UI ####
ui <- fluidPage(
  titlePanel("DNA Quant Plate Analysis"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  ...
  observeEvent(input$load_data, {
    ...
    data_combined <- read_quant_data(input$file_raw$datapath, input$file_plate$datapath, input$quant_kit)
  ...
  # ---------------------------------------------------------------------------
  # Updated output$model_grid now uses precomputed predictions from model_fit():
  # ---------------------------------------------------------------------------
  ...
  output$download_zip <- downloadHandler(
    filename = function() {
      req(input$file_raw, input$file_plate)
      prefix <- common_prefix(input$file_raw$name,
                              input$file_plate$name)
      prefix <- tolower(prefix)
      paste0(prefix, "_quantModel_", Sys.Date(), ".zip")
    },
    content = function(file) {
      req(data_all(), model_fit(),
          input$chosen_models, input$selected_standards)
      
      #### make temp directory and files ####
      prefix <- common_prefix(input$file_raw$name,
                              input$file_plate$name)
      prefix <- tolower(prefix)
      ...
      tmpdir <- tempdir()
      tmpdir <- str_c('outdir/', basename(tmpdir))
      dir.create(tmpdir, showWarnings = FALSE, recursive = TRUE)
      
      prediction_path <- file.path(tmpdir, 
                                   str_c(prefix, 
                                         "_sample_predictions_",
                                         Sys.Date(),
                                         '.csv'))
      inputs_path <- file.path(tmpdir, 
                               str_c(prefix, 
                                     "_inputs_",
                                     Sys.Date(),
                                     '.txt'))
      model_path <- file.path(tmpdir, 
                              str_c(prefix, 
                                    "_models_",
                                    Sys.Date(),
                                    '.rds'))
      code_path <- file.path(tmpdir, 
                             str_c(prefix, 
                                   "_sourceCode_",
                                   Sys.Date(),
                                   '.R'))
      
      plots_path <- file.path(tmpdir, 
                             str_c(prefix, 
                                   "_plots_",
                                   Sys.Date(),
                                   '.png'))
      ...
      message('DEBUG: tmpdir: ', 
              str_c(tmpdir, collapse = '; '))
      ...
      zip_out <- zip(zipfile = file, files = basename(files_to_zip))
