  ...
  tabsetPanel(
               ...
               mainPanel(
                 ...
                 div(
                   ...
                   tags$ul(
                     ...
                     tags$li("Go to ", tags$code("Calculate Mean DNA Concentrations"), " Tab")
                   )
                 ),
                 h4("Merged Data"),
    ...
    tabPanel("Calculate Mean DNA Concentrations",
             sidebarLayout(
               ...
               mainPanel(
                 # Instructions for Calculate Mean DNA Concentrations tab
                 div(
                   style = "background-color: #f8f9fa; border: 1px solid #dee2e6; border-radius: 5px; padding: 15px; margin-bottom: 20px;",
                   h4("Instructions:", style = "margin-top: 0; color: #495057;"),
                   tags$ul(
...
server <- function(input, output, session) {
  ...
  output$dna_plot <- renderPlot({
    if (is.null(joined_data()) || (length(joined_data()) > 0 && nrow(joined_data()) == 0)) {
      # No data loaded yet
      ggplot() + 
        annotate("text", x = 0.5, y = 0.5, 
  ...
  observeEvent(input$generate_zip, {
    ...
    prefix <- tolower(common_prefix(input$excel_file$name[1], input$csv_files$name[1]))
