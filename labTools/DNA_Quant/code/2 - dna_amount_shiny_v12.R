...
compute_empirical_priors <- function(y, is_control) {
  y_pos <- y[y > 0 & !is.na(y)]
  log_y <- log(y_pos)
  
  # Use non-controls for the intercept location
  is_ctrl_pos <- is_control[y > 0 & !is.na(y)]
  ...
  list(
    ...
    beta_prior           = c(0, 2.5),
    var_prior            = c(2, 1),
    inteceptShape_prior  = c(log(shape_est), 1.5),   # NB: matches Stan's typo
    ...
    tryCatch({
      ...
      if (!result$diag$ok) {
        ...
        warm_inits <- inits_from_fit(prev_fit,
                                     n_chains   = max(input$num_chains, 6),
                                     n_groups_1, n_groups_2)
        
        result <- run_attempt(
          num_chains    = max(input$num_chains, 6),
