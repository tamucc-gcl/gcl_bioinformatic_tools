...
functions {
  ...
  real partial_log_lik_lpmf(array[] int seq, int start, int end, data vector Y, data matrix Xc, vector b, real Intercept, data matrix Xc_shape, vector b_shape, real Intercept_shape, data array[] int J_1, data vector Z_1_1, vector r_1_1) {
    real ptarget = 0;
    int N = end - start + 1;
...
parameters {
  ...
  array[M_1] vector[N_1 - 1] z_1_raw;  // standardized group-level effects - sum to zero
}

transformed parameters {
  //chunk to reconstruct full vector post sum-to-zero
  array[M_1] vector[N_1] z_1;
  z_1[1, 1:(N_1 - 1)] = z_1_raw[1];
  z_1[1, N_1] = -sum(z_1_raw[1]);
...
model {
  ...
  if (!prior_only) {
    target += reduce_sum(partial_log_lik_lpmf, seq, grainsize, Y, Xc, b, Intercept, Xc_shape, b_shape, Intercept_shape, J_1, Z_1_1, r_1_1);
  }
  // priors including constants
  target += lprior;
  target += std_normal_lpdf(z_1_raw[1]); // sum to zero change
}
